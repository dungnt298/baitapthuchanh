...
def thuc_hien_yeu_cau(df):
    ...
    trung_binh_sv_dat = df.groupby('Mã lớp')[
        ['Loại A', 'Loại B+', 'Loại B', 'Loại C+', 'Loại C', 'Loại D+', 'Loại D', 'Loại F']].sum()
...
def thuc_hien():
    ...
    if file_path:
        try:
            ...
            tong_sv, tong_sv_dat_A, ti_le_dat_A, lop_max_A, trung_binh_sv_dat, tong_sv_dat_cua_tat_ca_lop, trung_binh_sv_dat_A_cua_tat_ca_lop = thuc_hien_yeu_cau(
                df)

            # Hiển thị kết quả
            ket_qua_label.config(
                text=f"Tổng số sinh viên: {tong_sv}\nTổng số sinh viên đạt điểm A: {tong_sv_dat_A}\nTỉ lệ sinh viên đạt điểm A: {ti_le_dat_A:.2f}%\n"
